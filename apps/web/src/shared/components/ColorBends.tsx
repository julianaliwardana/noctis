...
type ColorBendsUniforms = {
  ...
  uIterations: THREE.IUniform<number>;
  uIntensity: THREE.IUniform<number>;
  uBandWidth: THREE.IUniform<number>;
};

function uniformsOf(material: THREE.ShaderMaterial): ColorBendsUniforms {
  return material.uniforms as unknown as ColorBendsUniforms;
}
...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      uniformsOf(material).uCanvas.value.set(w, h);
    };
  ...
  useEffect(() => {
    ...
    const toVec3 = (hex: string) => {
      ...
      const r = h.length === 3 ? h.charAt(0) + h.charAt(0) : h.slice(0, 2);
      const g = h.length === 3 ? h.charAt(1) + h.charAt(1) : h.slice(2, 4);
      const b = h.length === 3 ? h.charAt(2) + h.charAt(2) : h.slice(4, 6);
      return new THREE.Vector3(parseInt(r, 16) / 255, parseInt(g, 16) / 255, parseInt(b, 16) / 255);
    };

    const arr = (colors || []).filter(Boolean).slice(0, MAX_COLORS).map(toVec3);
    const uColorsArray = uniforms.uColors.value;
    for (let i = 0; i < MAX_COLORS; i++) {
      const vec = uColorsArray[i]!;
      const next = arr[i];
      if (next) vec.copy(next);
      else vec.set(0, 0, 0);
    }
